...
describe("test POST /api/media/add route and the addMedia controller", () => {
    ...
    test("addMedia with valid inputs ", async (done) => {
        const response = await request(app).post('/api/media/add')
        ...
        const deleteResponse = await request(app).post('/api/media/delete')
            .set('Authorization', 'Bearer ' + token)
...
describe("test GET /api/media/ route and the serveMedia controller", () => {
    ...
    test("retrieveMedia with valid inputs", async (done) => {
        ...
        const retrieveResponse = await request(app).get('/api/media/')
            .set('Authorization', 'Bearer ' + token)
            .send(payload);

        let buffer = new Buffer(retrieveResponse.body.b64media, 'base64');
    ...
    test("retrieveMedia with missing media id", async (done) => {
        ...
        expect(retrieveResponse.text).toBe('Media retrieval failed - No media id provided')
...
describe("test POST /api/media/delete route and the deleteMedia controller", () => {
    ...
    test("deleteMedia", async (done) => {
        ...
        expect(deleteResponse.text).toBe('Media deletion success - deleted '+response.body._id);
        done();
    });
});
